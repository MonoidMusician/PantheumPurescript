...
update msg model =
    case msg of
        Next ->
            case model of
                SeekingInput content ->
                    content # lines # map rescan # Parsing
...
viewword :: Word -> Float -> Html Msg
viewword word index =
    viewsyllables word.syllables index # mapLeft (span [ title word.gloss ])


viewsyllables :: Array Syllable -> Float -> Array (Html Msg)
viewsyllables syllables index =
    syllables # statemap viewsyllable [] # mapLeft (Array.intersperse hyphen)
...
lines :: String -> Array Line
lines content =
    String.trim content # String.lines # map (String.trim >>> words)


words :: String -> Array Res
words content =
        ...
        inner syllables bias =
            let
                ...
                next ( syllable, stype ) ( res, bias ) =
                    ( (if bias == 0 && Regex.contains r_elision syllable && Array.isEmpty res then
                        ( syllable, Elided )
                       else if bias == 2 then
                        ( syllable, Long )
                       else
            ...
            in
                Array.foldr next start syllables
...
syllables :: String -> Array Syllable
syllables content =
